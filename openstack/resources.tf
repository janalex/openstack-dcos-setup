...
resource "template_file" "cloud-init-master" {
    ...
    vars {
        ...
        repository_url = "${var.dcos_repository_url}"
    }
}
...
resource "openstack_compute_instance_v2" "dcos-master" {
    ...
    network {
        ...
        fixed_ip_v4 = "${var.openstack_subnet_ip_prefix}.${var.openstack_subnet_master_ip_suffix}"
    }
    ...
    connection {
        host = "${openstack_compute_floatingip_v2.master-fip.address}"
    ...
    provisioner "remote-exec" {
        inline = [
            "sudo /bin/bash -c 'echo \"${var.openstack_subnet_ip_prefix}.${var.openstack_subnet_master_ip_suffix} dcos-master dcos-master.novalocal\" > /etc/hosts'",
            "sudo /bin/bash -c 'echo \"COREOS_PRIVATE_IPV4=${var.openstack_subnet_ip_prefix}.${var.openstack_subnet_master_ip_suffix}\" > /etc/environment'",
            "sudo /bin/bash -c 'echo \"COREOS_PUBLIC_IPV4=${self.floating_ip}\" >> /etc/environment'",
            "sudo /bin/bash -c 'echo \"EXHIBITOR_HOSTNAME=${var.openstack_subnet_ip_prefix}.${var.openstack_subnet_master_ip_suffix}\" >> /etc/mesosphere/setup-packages/dcos-config--setup/etc/exhibitor'",
        ]
    }
...
resource "openstack_compute_instance_v2" "dcos-slave" {
    ...
    network {
        uuid = "${openstack_networking_network_v2.dcos-network.id}"
        fixed_ip_v4 = "${var.openstack_subnet_ip_prefix}.${format(\"%d\", count.index + var.openstack_subnet_master_ip_suffix + 1)}"
    }

    user_data = "${template_file.cloud-init-slave.rendered}"

    connection {
        host = "${var.openstack_subnet_ip_prefix}.${format(\"%d\", count.index + var.openstack_subnet_master_ip_suffix + 1)}"
        user = "${var.openstack_ssh_user}"
        key_file = "${var.openstack_ssh_private_key_file}"
    ...
    provisioner "remote-exec" {
        inline = [
            "sudo /bin/bash -c 'echo \"${var.openstack_subnet_ip_prefix}.${format(\"%d\", count.index + var.openstack_subnet_master_ip_suffix + 1)} ${self.name} ${self.name}.novalocal\" > /etc/hosts'",
            "sudo /bin/bash -c 'echo \"COREOS_PRIVATE_IPV4=${var.openstack_subnet_ip_prefix}.${format(\"%d\", count.index + var.openstack_subnet_master_ip_suffix + 1)}\" > /etc/environment'",
            "sudo /bin/bash -c 'echo \"MESOS_HOSTNAME=${var.openstack_subnet_ip_prefix}.${format(\"%d\", count.index + var.openstack_subnet_master_ip_suffix + 1)}\" >> /etc/mesosphere/setup-packages/dcos-config--setup/etc/mesos-slave'",
            "sudo /bin/bash -c 'echo \"MASTER_ELB=${var.openstack_subnet_ip_prefix}.${var.openstack_subnet_master_ip_suffix}\" >> /etc/mesosphere/setup-packages/dcos-config--setup/etc/cloudenv'",
        ]
    }
